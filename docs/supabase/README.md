...
- `012_coach_network.sql`, Coach Network (find-coach, basvuru, teklif, mesajlasma, remote athlete, `coaching_program_assignments`, `training_proofs`, `coaching-proofs` storage bucket, review) icin tablolari kurar. Coach Network ozellikleri acilmadan once Supabase SQL Editor'de calistirin. CN4 program atama `coaching_program_assignments` kullanir; CN5 proof yukleme `training_proofs` + private bucket `coaching-proofs` (signed URL).
